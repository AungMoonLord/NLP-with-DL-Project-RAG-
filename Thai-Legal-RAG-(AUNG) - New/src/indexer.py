...
class Indexer:
    def __init__(self):
        ...
        # กำหนดความยาวสูงสุดของโมเดลตรงนี้ (แก้ปัญหา Sequence เกิน 512 tokens)
    ...
    def build(self, chunks: list[dict], out_dir: str = config.INDEX_DIR):
        ...
        embeddings = self.model.encode(
            ...
            #max_length=512,      # กำหนดไม่ให้เกิน Sequence length สูงสุดของ e5-base
            #runcation=True,     # ตัด token ส่วนเกินทิ้งอัตโนมัติหากเกิน 512
        ).astype("float32")
